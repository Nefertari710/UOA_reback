            ...
            while (true)
            {
                ...
                if (command == 3) {
                    ...
                    using (NetworkStream stream = tcpClientCache3.GetStream())
                    {
                        stream.Write(combinedData, 0, combinedData.Length);
                        stream.Flush();
                        Invoke((Action)(() => labelShowStatus3.Text = "Best Wish"));
                        ...
                        for (int i = 0; i < PieceLengthBytes; i++)
                        {
                            ...
                            byte[] PieceType = new byte[4];
                            stream.Read(PieceType, 0, PieceType.Length);
                            int PieceTypeInt = BitConverter.ToInt32(PieceType, 0);

                            // 在这个地方我们需要接收文件的长度 HashPieceLengthInt
                            byte[] HashPieceLength = new byte[4];
                            stream.Read(HashPieceLength, 0, PieceType.Length);
                            ...
                            if (PieceTypeInt == 1)
                            {
                                ...
                                string PieceDataFilePath = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "../../../CachePieceData", fileName, HashPieceContentString1.ToString()));
                                File.WriteAllBytes(PieceDataFilePath, HashPieceContentBytes);
                                Invoke((Action)(() => labelShowStatus3.Text = "文件不存在"));
